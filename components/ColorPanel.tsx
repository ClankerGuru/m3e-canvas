...
import { CONTRASTS, PALETTES } from "@/lib/tokens";
import type { Contrast, Palette, Theme } from "@/lib/tokens";
...
function Swatches({ pal, p }: { pal: Palette; p: Palette }) {
  ...
  return (
    <span style={s({ display: "inline-flex", flex: "0 0 auto", marginLeft: "auto", paddingLeft: 8 })}>
      {colors.map((c, i) => (
        <span
          key={i}
          style={s({
...
function ColorField({ value, onChange, p, label }: { value: string; onChange: (hex: string) => void; p: Palette; label: string }) {
  ...
  return (
    <label style={s({ display: "flex", alignItems: "center", gap: 8, height: 36 })}>
      <span style={s({ position: "relative", width: 28, height: 28, borderRadius: 14, background: value, boxShadow: "inset 0 0 0 1px rgba(0,0,0,0.12)", flex: "0 0 auto", overflow: "hidden" })}>
        <input
          type="color"
          value={value}
          onChange={(e) => onChange(e.target.value.toUpperCase())}
          aria-label={label}
          style={s({ position: "absolute", inset: -8, width: 44, height: 44, opacity: 0, cursor: "pointer" })}
        />
      </span>
      <span style={s({ flex: 1, minWidth: 0, fontSize: 12, color: p.onSurface, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" })}>{label}</span>
      <input
        value={text()}
        onChange={(e) => {
...
function SettingRow({
  ...
  return (
    ...
    >
      <span style={s({ width: 28, height: 28, borderRadius: 14, background: p.secondaryContainer, color: p.onSecondaryContainer, display: "grid", placeItems: "center", flex: "0 0 auto" })}>
        <Icon name={icon} size={18} />
      </span>
      <span style={s({ flex: 1, minWidth: 0, fontSize: 13, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" })}>{label}</span>
      {children}
    </button>
  );
}
    ...
    <div class="no-scrollbar" style={s({ height: "100%", overflowY: "auto", padding: "12px 12px 20px" })}>
      <div style={s({ display: "flex", flexDirection: "column", gap: 6, marginBottom: 14 })}>
        <SettingRow p={p} icon={theme.dark ? "dark_mode" : "light_mode"} label={t("brightness", lang)} onClick={() => onTheme({ dark: !theme.dark })} pressed={theme.dark}>
          <span style={s({ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 13, fontWeight: 600, color: p.primary })}>
      ...
      {tab() === "templates" ? (
        <div style={s({ display: "flex", flexDirection: "column", gap: 6, marginTop: 12 })}>
          {PALETTES.map((pal) => {
            ...
            return (
              ...
              >
                <span style={s({ width: 28, height: 28, borderRadius: 14, background: `linear-gradient(135deg, ${pal.primary} 50%, ${pal.primaryContainer} 50%)`, flex: "0 0 auto" })} />
                <span style={s({ fontSize: 13, fontWeight: 600, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", minWidth: 0 })}>{pal.label}</span>
        ...
        <div style={s({ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 })}>
          <div
            style={s({
              display: "flex",
              alignItems: "center",
          ...
          >
            <div style={s({ flex: "1 1 180px", minWidth: 0 })}>
            ...
            {paletteKey !== "custom" && (
              <button
                ...
                class="m3-press"
                style={s({ height: 32, padding: "0 12px", borderRadius: 16, border: "none", background: p.primary, color: p.onPrimary, fontSize: 12, fontWeight: 600, cursor: "pointer" })}
              >
                {t("useThis", lang)}
              </button>
            )}
          </div>
          <div style={s({ fontSize: 11, lineHeight: 1.5, color: p.onSurfaceVariant, padding: "0 4px" })}>{t("seedHint", lang)}</div>
          <div style={s({ display: "flex", gap: 6, flexWrap: "wrap", padding: "0 2px" })}>
            {["#6750A4", "#0B57D0", "#2E6A45", "#984061", "#8B5000", "#00696E", "#B3261E", "#4A4459"].map((c) => (
              <button
                ...
                aria-label={c}
                class="m3-press"
                style={s({ width: 26, height: 26, borderRadius: 13, border: "none", padding: 0, background: c, cursor: "pointer", outline: seed() === c ? `2px solid ${p.primary}` : "2px solid transparent", outlineOffset: 2 })}
              />
            ))}
          </div>
      ...
      <div style={s({ marginTop: 16, display: "flex", flexDirection: "column", gap: 6 })}>
        <SettingRow p={p} icon="wallpaper" label={t("dynamicColor", lang)} onClick={() => onDynamic(!dynamic)} pressed={dynamic}>
          <Knob on={dynamic} p={p} />
        </SettingRow>
        <div style={s({ fontSize: 11, lineHeight: 1.5, color: p.onSurfaceVariant, padding: "2px 4px 0" })}>
