...
const ORIG_CS = globalThis.CompressionStream;
const ORIG_DS = globalThis.DecompressionStream;
afterEach(() => {
  (globalThis as any).CompressionStream = ORIG_CS;
  (globalThis as any).DecompressionStream = ORIG_DS;
});
...
describe("shareLink and readShareHash", () => {
  ...
  it("accepts a legacy document without adding defaults", async () => {
    ...
    await expect(readShareHash(plainHash(legacy))).resolves.toEqual(legacy as unknown as Doc);
  });
