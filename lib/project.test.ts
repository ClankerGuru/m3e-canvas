...
describe("readProject", () => {
  ...
  it("preserves a legacy file and unknown fields for the editor's migration step", async () => {
    ...
    await expect(readProject(new File([JSON.stringify(legacy)], "old.json"))).resolves.toEqual(legacy as unknown as Doc);
  });
  ...
  it("returns null when reading the File fails", async () => {
    ...
    const read = spyOn(file, "text").mockRejectedValue(new Error("Read failed"));
    try {
