...
- Run `bun run typecheck` and `bun run build`; CI runs the same two on every PR.
...
- PR の前に `bun run typecheck` と `bun run build` を通してください。CI でも同じものが走ります。
...
- 提交 PR 前请运行 `bun run typecheck` 和 `bun run build`，CI 会执行同样的检查。
- 贡献的代码以 MIT 许可证发布。
